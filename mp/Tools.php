...
class Tools{
    ...
    static function ConvertTimestamp($timestamp){
        return date_create($timestamp)->setTimezone(new DateTimeZone('Asia/Shanghai'));
    }
    
    static function GetEventUsers($eventUsers){
    ...
    static function ParseEvent($event, $users, $msgId){
        $time = '['.static::ConvertTimestamp($event->timestamp)->format('Y-m-d H:i:s').']';
        switch($event->detail->type){
            ...
            case 'match-disbanded':
                return $time.' 房间被关闭';
            case 'other':
                $img = static::DrawMatchEvent($event, $users);
                $img->save(DataStorage::$storagePath."data/osu.mp/-{$msgId}");
                return sendImg(DataStorage::GetData("osu.mp/-{$msgId}"));
            default:
                return '';
        }
    }
    
    static function drawMatchEvent($event, $users){
        $game = $event->game;
        $beatmap = $game->beatmap;
        $scores = $game->scores;
        $here = __DIR__.'/';
        $outside = $here.'../';
        $assets = $outside.'resources/';
        $mpAssets = $assets.'mp/';
        $exo2 = $assets.'Exo2-Regular.ttf';
        $exo2_italic = $assets.'Exo2-Italic.ttf';
        $exo2_bold = $assets.'Exo2-Bold.ttf';
        $yahei = $assets.'Yahei.ttf';
        $white = '#ffffff';
        $startTime = static::ConvertTimestamp($game->start_time)->format('H:i');
        $endTime = static::ConvertTimestamp($game->end_time)->format('H:i');
        $teamTypeImg = Image::make(SVG::fromFile($mpAssets.$game->team_type.'.svg')->toRasterImage(40, 40));
        try{
        ...
        switch($game->team_type){
            ...
            case 'team-vs':
                ...
                foreach($scores as $score){
                    if($score->match->team == 'red')$redScore += $score->score;
                    if($score->match->team == 'blue')$blueScore += $score->score;
                }
        ...
        foreach($scores as $score){
            $eventResult->insert(static::DrawPlayerMatchScore($score, $users[$score->user_id], $game->mode), 'top-left', $xIndex, $yIndex);
            $yIndex+=79;
        }
    ...
    static function DrawPlayerMatchScore($score, $user, $mode){
        $here = __DIR__.'/';
        $outside = $here.'../';
        $assets = $outside.'resources/';
        $mpAssets = $assets.'mp/';
        ...
        $team = Image::make($mpAssets.$score->match->team.'.png')->resize(40, 74);
        $flag = Image::make($assets."flags/{$user->country_code}.png")->resize(30, 20);
